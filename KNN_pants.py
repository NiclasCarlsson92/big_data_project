...
def main():
    ...
    nearest = []
    if user_sex == 0:
        for length, weight in zip(pants_data_m['stature'], pants_data_m['weightkg']):
        ...
        for length, weight in zip(pants_data_f['stature'], pants_data_f['weightkg']):
            delta_y = abs(user_length) - abs(length)
            delta_x = abs(user_weight) - abs(weight)
            hypotenuse = delta_y ** 2 + delta_x ** 2
            nearest.append(math.sqrt(hypotenuse))
    ...
    if user_sex == 0:
        pants_data_m['nearest'] = nearest
    else:
        pants_data_f['nearest'] = nearest

    if user_sex == 0:
        X = pants_data_m.drop(columns=['size', 'color'])
        Y = pants_data_m['size']
    else:
        X = pants_data_f.drop(columns=['size', 'color'])
        Y = pants_data_f['size']
    ...
    if user_sex == 0:
        ...
        print(pants_data_m.sort_values(by=['nearest']).head(best_k))
        print(pants_data_m.iat[0, 5], 'is your size!!')
    else:
        print(f'Accuracy: {int(accuracy)}%')
        print(pants_data_f.sort_values(by=['nearest']).head(best_k))
        print(pants_data_f.iat[0, 5], 'is your size!!')
